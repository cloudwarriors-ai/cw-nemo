...
node --input-type=module -e '
  ...
  const workerAgentId = process.argv[5];
  const summary = JSON.parse(readFileSync(`${reviewRoot}/project-summary.json`, "utf8"));
  const workers = JSON.parse(readFileSync(`${reviewRoot}/project-workers.json`, "utf8"));
  ...
  writeFileSync(process.argv[6], `${lines.join("\n")}\n`, "utf8");
  console.log(lines.join("\n"));
' "$REVIEW_ROOT" "$PROJECT_ID" "$TASK_ID" "$WORKER_ID" "$WORKER_AGENT_ID" "$REPORT_PATH"
